// THD Curve Shape Analysis
// Real tape: THD roughly quadruples per +6dB (ratio ~4x per +6dB, ~2x per +3dB)
...
int main() {
    double sr = 96000;
    std::cout << "THD CURVE SHAPE ANALYSIS\n";
    std::cout << "========================\n\n";
    std::cout << "Real tape: THD roughly quadruples per +6dB (ratio ~4x)\n";
    std::cout << "           This means ~2x per +3dB\n\n";
    ...
    for (int m = 0; m < 2; ++m) {
        ...
        std::cout << (m==0 ? "AMPEX ATR-102:\n" : "\nSTUDER A820:\n");
        std::cout << "Level      THD%      Ratio vs prev    Ideal (2x/+3dB)\n";
        std::cout << "------------------------------------------------------\n";

        double prevTHD = 0;
        for (double lv = -12; lv <= 15; lv += 3) {
            ...
            if (lv > -12) std::cout << std::setw(6) << std::setprecision(2) << ratio << "x          2.0x";
            std::cout << "\n";
            prevTHD = thd;
        }
    }
