...
int main() {
    ...
    std::cout << "=== AMPEX ATR-102 Tuning (100Hz) ===\n\n";
    std::cout << "Targets:\n";
    std::cout << "  - THD @ 0dB: 0.3-0.5%\n";
    std::cout << "  - MOL (3% THD): +12dB\n";
    std::cout << "  - E/O ratio: ~0.5 (odd-dominant)\n\n";

    double testLevels[] = {-12.0, -6.0, 0.0, 3.0, 6.0, 9.0, 12.0, 15.0, 18.0};
    ...
    for (double levelDb : testLevels) {
        ...
        const char* status = "";
        if (levelDb == 0.0) {
            if (thd >= 0.3 && thd <= 0.5) status = "✓ ON TARGET";
            else if (thd < 0.3) status = "⬆ need more THD";
            else status = "⬇ need less THD";
        } else if (thd >= 2.9 && thd <= 3.1 && !foundMol) {
            status = "← MOL (3% THD)";
            molLevel = levelDb;
            foundMol = true;
        }
    ...
    std::cout << "\n";
    if (!foundMol) {
        // Find interpolated MOL
        std::cout << "MOL not exactly hit - interpolating...\n";
    }
    ...
    // Check E/O ratio at +6dB
    {
        ...
        std::cout << "E/O ratio @ +6dB: " << eoRatio << " (target: ~0.5)\n";
        if (eoRatio < 1.0) std::cout << "  ✓ Odd-dominant as expected\n";
        else std::cout << "  ✗ Should be odd-dominant (< 1.0)\n";
    }
